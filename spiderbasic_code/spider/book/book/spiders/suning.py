...
class SuningSpider(scrapy.Spider):
        ...
        for div in div_list:
            ...
            for p in p_list:
                ...
                for li in li_list:
                    ...
                    # 发送请求的，获取列表页第一页后部分的数据`
                    next_part_url = 'https://list.suning.com/emall/showProductList.do?ci={}&pg=03&cp=0&il=0&iy=0&adNumber=0&n=1&ch=4&prune=0&sesab=ACBAAB&id=IDENTIFYING&cc=010&paging=1&sub=0'
                    # 获取url地址的ci,ci值在当前页请求url的参数
    ...
    def parse_book_list(self,response):
        ...
        for li in li_list:
            ...
            item["book_follow"] = li.xpath(".//p[@class='com-cnt']/a/text()").extract_first()
            item["book_href"] = li.xpath(".//p[@class='sell-point']/a/@href").extract_first()
            item["bookstore_name"] = li.xpath(".//p[contains(@class,'seller oh no-more')]/a/text()").extract_first()
            # print(item)
            # 发送详情页的请求
